...
export default async function InsightsPage({ params }) {
  ...
  return (
    <>
      ...
      <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16 pt-2">
        <div className="path-indicator inline-flex items-center gap-2 text-accent border border-accent/30 bg-accent/10 rounded-base px-3.5 py-1 mb-4 font-semibold">
          <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
          <span>{minYear} → {maxYear} · {en ? `${yearsCovered} years of evolution` : `${yearsCovered} ans d'évolution`}</span>
        </div>
        <h1 className="font-display font-bold text-3xl sm:text-5xl leading-tight mb-3 text-fg">
          <span className="text-brand-gradient">{en ? 'Earbuds Insights' : 'Analyses & Tendances'}</span>
        </h1>
      ...
      {/* Tuiles de stats Sonic Chronology */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-14">
        {batteryGrowth && (
          <div className="hardware-card bg-panel p-6">
            <div className="path-indicator text-accent mb-2.5">
              {en ? 'Section 01 — Battery life' : 'Section 01 — Autonomie'}
            </div>
            <div className="flex items-baseline justify-between gap-4 mb-2">
              <h2 className="font-display font-bold text-lg text-fg leading-snug">
                {en ? 'Wireless earbuds last longer.' : 'Les écouteurs tiennent plus longtemps.'}
              </h2>
              <span className="font-display font-bold text-4xl text-accent signal-glow shrink-0">
        ...
        {priceGrowth && (
          <div className="hardware-card bg-panel p-6">
            ...
            <div className="flex items-baseline justify-between gap-4 mb-2">
              ...
              <span className="font-display font-bold text-4xl text-fg shrink-0">
        ...
        {brandShare.length > 0 && (
          <div className="hardware-card bg-panel p-6">
            ...
            <h2 className="font-display font-bold text-lg text-fg leading-snug mb-3">
              {en ? 'Who shaped the market?' : 'Qui a façonné le marché ?'}
            </h2>
            <div className="flex flex-col gap-2.5">
              {brandShare.map((b) => (
                <div key={b.id} className="flex items-center gap-3">
                  <span className="text-xs font-mono text-dim w-24 shrink-0 truncate">{b.name}</span>
                  <div className="flex-1 h-2 bg-panel2 rounded-base overflow-hidden border border-line">
                    <div className="h-full bg-accent" style={{ width: `${b.pct}%` }} />
                  </div>
                  <span className="text-xs font-mono text-fg font-semibold w-9 text-right shrink-0">{b.pct}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {weightGrowth && (
          <div className="hardware-card bg-panel p-6">
            ...
            <div className="flex items-baseline justify-between gap-4 mb-2">
              ...
              <span className={`font-display font-bold text-4xl shrink-0 ${weightGrowth.pct < 0 ? 'text-accent signal-glow' : 'text-fg'}`}>
