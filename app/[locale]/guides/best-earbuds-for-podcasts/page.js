...
const score = (m) => Math.min(100, 50 + (m.anc ? 15 : 0) + (m.transparency_mode || m.transparency ? 8 : 0) + ((n(m.battery_bud_h) || 0) >= 7 ? 10 : ((n(m.battery_bud_h) || 0) >= 5 ? 5 : 0)) + ((n(m.weight_g) || 99) <= 7 ? 7 : 0) + (String(m.bluetooth_codecs || m.codecs || '').trim() ? 5 : 0));
  ...
  const card = (r) => (
    <Link key={r.model.id} href={`/${locale}/ecouteurs/${r.model.id}`} className="block bg-panel border border-line rounded-2xl p-5 hover:border-accent">
      ...
      <div className="flex justify-between gap-4 mt-1">
        ...
        <div className="text-right"><b>{price(r.model.price)}</b><div className="font-mono text-[9px] text-accent">{r.score}/100</div></div>
      </div>
      <div className="mt-4 text-[10px] font-mono text-dim">
        {r.model.anc ? 'ANC · ' : ''}{r.model.transparency_mode || r.model.transparency ? 'Transparency · ' : ''}{n(r.model.battery_bud_h) != null ? `${n(r.model.battery_bud_h)}h` : 'Battery —'}
      </div>
    </Link>
  );
    ...
    <>
      ...
      <article className="max-w-6xl mx-auto">
        ...
        <section className="mt-12">
          <h2 className="font-display font-semibold text-[25px]">💵 {fr ? 'Meilleurs écouteurs pour podcasts sous 100 $' : 'Best Podcast Earbuds Under $100'}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-5">{budget.map(card)}</div>
        </section>
        <section className="mt-12">
          <h2 className="font-display font-semibold text-[25px]">{fr ? 'Ce qui compte pour les voix' : 'What matters for spoken audio'}</h2>
          <p className="text-dim text-sm leading-7 max-w-3xl mt-3">{fr ? 'Pour les podcasts, une restitution claire des voix, une bonne isolation et une autonomie suffisante peuvent être plus importantes qu’une restitution spectaculaire des graves. La qualité réelle dépend toutefois du modèle, de l’enregistrement et de l’écoute.' : 'For podcasts, clear voice reproduction, good isolation and sufficient battery life can matter more than a dramatic bass response. Real-world quality still depends on the model, recording and listening experience.'}</p>
        </section>
        <section className="mt-12">
          <h2 className="font-display font-semibold text-[25px]">{fr ? 'ANC et podcasts' : 'ANC and podcasts'}</h2>
          <p className="text-dim text-sm leading-7 max-w-3xl mt-3">{fr ? 'L’ANC peut être particulièrement pratique dans les transports ou les environnements bruyants, car réduire certains bruits ambiants peut faciliter la compréhension des voix.' : 'ANC can be particularly useful on public transport or in noisy environments because reducing some ambient noise can make voices easier to follow.'}</p>
        </section>
        <section className="mt-12">
          <h2 className="font-display font-semibold text-[25px]">{fr ? 'Méthodologie' : 'Methodology'}</h2>
          <p className="text-dim text-sm leading-7 max-w-3xl mt-3">{fr ? 'Le score combine ANC, transparence, autonomie, poids et disponibilité de données sur les codecs. Il ne mesure pas directement l’intelligibilité des voix ni la qualité sonore réelle.' : 'The score combines ANC, transparency, battery life, weight and available codec data. It does not directly measure speech intelligibility or real-world sound quality.'}</p>
        </section>
        <section className="mt-12 border border-line rounded-2xl p-6 bg-panel">
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-2 text-sm">
            <Link className="hover:text-accent" href={`/${locale}/guides/best-earbuds-for-calls`}>→ Best Earbuds for Calls</Link>
            <Link className="hover:text-accent" href={`/${locale}/guides/best-earbuds-for-working`}>→ Best Earbuds for Working</Link>
            <Link className="hover:text-accent" href={`/${locale}/guides/best-earbuds-for-commuting`}>→ Best Earbuds for Commuting</Link>
            <Link className="hover:text-accent" href={`/${locale}/guides/best-noise-cancelling-earbuds`}>→ Best Noise Cancelling Earbuds</Link>
          </div>
        </section>
      </article>
      <Footer />
