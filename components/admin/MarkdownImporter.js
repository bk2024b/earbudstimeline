...
export default function MarkdownImporter({ models = [], brands = [], locale = 'fr', onImport }) {
  ...
  const analysis = parsedResult?.analysis || { mainSections: 0, subSections: 0, sectionWordCounts: {}, warnings: [] };
      ...
      {!parsedResult ? (
        ...
        <div className="bg-panel2 rounded-lg p-3 border border-line flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Stat label="Sections principales (H2)" value={analysis.mainSections} />
            <Stat label="Sous-sections (H3-H4)" value={analysis.subSections} />
          </div>

          {analysis.warnings.length > 0 && (
            <div className="border-t border-line/50 pt-3">
              <p className="text-xs text-amber font-semibold mb-1.5">⚠ Points d’attention</p>
              <ul className="space-y-1 pl-0 list-none">
                {analysis.warnings.map((warning, i) => (
                  <li key={i} className="text-[11px] text-dim flex gap-1.5">
          ...
          <div className="border-t border-line/50 pt-3">
            ...
            {toc.length > 0 ? (
              <div className="max-h-56 overflow-auto space-y-1 pr-1">
                {toc.map((item) => (
                  <div key={item.id} className="flex items-start justify-between gap-2 text-xs" style={{ paddingLeft: `${Math.max(0, item.level - 2) * 16}px` }}>
                    <div className="flex items-start gap-2 min-w-0">
                      <span className="text-accent font-mono text-[10px] mt-0.5 shrink-0">H{item.level}</span>
                      <span className="text-white line-clamp-2">{item.title}</span>
                    </div>
                    {analysis.sectionWordCounts[item.id] !== undefined && (
                      <span className="text-[10px] text-dim shrink-0 mt-0.5">{analysis.sectionWordCounts[item.id]} mots</span>
                    )}
                  </div>
                ))}
              </div>
