...
const required = ['earbud_name', 'value', 'source_url', 'noise_category'];
...
export default function AncCsvImportForm() {
  ...
  async function handleImport() {
    ...
    try {
      ...
      setResults([{ ok: false, earbud_name: '?', error: e.message }]);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="bg-panel border border-line rounded-xl p-5">
        ...
        <p className="text-xs text-dim mt-2">Une ligne = une preuve ANC. Le modèle est identifié par son <code>earbud_name</code> (nom exact dans la base), pas par son ID interne.</p>
        <pre className="mt-3 bg-panel2 border border-line rounded-lg p-3 overflow-x-auto text-[11px] text-dim">{`evidence_id,earbud_name,metric,value,measurement_type,measurement_context,noise_category,source_url,source_name,source_type,confidence,notes\n,AirPods Pro 3,anc,95,qualitative_environment,engine cabin traffic and conversations,airplane,https://…,RTINGS,laboratory,high,Notes`}</pre>
        <p className="text-[11px] text-dim mt-2">Les alias de <code>source_type</code> comme <code>review</code> sont automatiquement convertis en <code>editorial_test</code>.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <a
          href="data:text/csv;charset=utf-8,evidence_id%2Cearbud_name%2Cmetric%2Cvalue%2Cmeasurement_type%2Cmeasurement_context%2Cnoise_category%2Csource_url%2Csource_name%2Csource_type%2Cconfidence%2Cnotes%0A%2CAirPods%20Pro%203%2Canc%2C95%2Cqualitative_environment%2Cairplane%20cabin%2Cairplane%2Chttps%3A%2F%2Fexample.com%2CRTINGS%2Claboratory%2Chigh%2C"
            ...
            <div className="max-h-[460px] overflow-auto">
              <table className="w-full text-xs min-w-[800px]">
                <thead className="bg-panel2 text-dim sticky top-0">
                  <tr><th className="text-left px-3 py-2">Ligne</th><th className="text-left px-3 py-2">Modèle</th><th className="text-left px-3 py-2">Valeur</th><th className="text-left px-3 py-2">Environnement</th><th className="text-left px-3 py-2">Source</th><th className="text-left px-3 py-2">Validation</th></tr>
                </thead>
                <tbody>
                  {validation.map((v, i) => (
                    <tr key={i} className="border-t border-line">
                      <td className="px-3 py-2">{i + 2}</td>
                      <td className="px-3 py-2">{v.row.earbud_name || '—'}</td>
      ...
      {results && <div className="border-t border-line pt-4 flex flex-col gap-1">{results.map((r, i) => <div key={i} className={`text-xs ${r.ok ? 'text-emerald-400' : 'text-rose-400'}`}>{r.ok ? '✓' : '✗'} {r.earbud_name}{r.error ? ` — ${r.error}` : ''}</div>)}</div>}
    </div>
  );
}
