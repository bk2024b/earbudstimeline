 ...
 * Retourne { sent, failed, lastError } — ne lève jamais d'exception : un échec
 * d'envoi pour un lot ne doit pas empêcher les autres lots de partir.
 * lastError conserve le message renvoyé par Resend pour le premier lot en
 * échec, pour qu'on sache POURQUOI plutôt que juste "ça a échoué".
 */
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!lastError) {
          ...
          lastError = `Resend a répondu ${res.status}${body ? ` — ${body.slice(0, 300)}` : ''}`;
        }
      }
    } catch (e) {
      failed += group.length;
