...
// Moyenne pondérée qui ignore les composantes absentes (evidence NULL) au
// lieu de les traiter comme 0 — un modèle avec seulement 2 environnements
// documentés sur 4 ne doit pas être pénalisé comme s'il avait 0 partout.
...
export function analyzeEarbudsForBudget(models, brands, {
  ...
  brandId = 'all',
  locale = 'fr',
  // Scores ANC fondés sur les preuves (vue earbuds_anc_scores), un par
  // environnement (Travel/Office/Traffic/Voices). Optionnel : les modèles
  // sans evidence retombent sur l'heuristique binaire m.anc ? 100 : 0.
  ancScores = [],
} = {}) {
  ...
  const scored = candidates.map((m) => {
    ...
    // Réduction de bruit active — fondée sur les preuves (evidence) quand
    // disponibles pour ce modèle, sinon repli sur l'heuristique binaire.
    const anc = ancMap[m.id] || {};
    const ancTravel = anc.anc_travel_score == null ? null : Number(anc.anc_travel_score);
    const ancOffice = anc.anc_office_score == null ? null : Number(anc.anc_office_score);
    const ancTraffic = anc.anc_traffic_score == null ? null : Number(anc.anc_traffic_score);
    const ancVoices = anc.anc_voices_score == null ? null : Number(anc.anc_voices_score);
    const evidenceAncScore = weightedAverage([
      { value: ancTravel, weight: 0.30 },
      { value: ancOffice, weight: 0.25 },
      { value: ancTraffic, weight: 0.25 },
      { value: ancVoices, weight: 0.20 },
    ]);
    const hasEvidence = anc.anc_score != null || evidenceAncScore != null;
    const ancScore = anc.anc_score != null
      ? Number(anc.anc_score)
      : evidenceAncScore != null
        ? evidenceAncScore
        : (m.anc ? 100 : 0);
    const ancCoverage = anc.coverage_score != null
      ? Number(anc.coverage_score)
      : anc.environment_count
        ? (Number(anc.environment_count) / 4) * 100
        : 0;
    ...
    switch (priority) {
      case 'anc':
        score = (ancScore * 0.40) + (ancCoverage * 0.10) + (batteryScore * 0.15) + (comfortScore * 0.10) + (recencyBonus * 0.15) + (valueScore * 0.10);
        break;
      case 'battery':
    ...
    return {
      ...
      metrics: {
        ...
        ancHasEvidence: hasEvidence,
        ancTravel: ancTravel == null ? null : Math.round(ancTravel),
        ancOffice: ancOffice == null ? null : Math.round(ancOffice),
        ancTraffic: ancTraffic == null ? null : Math.round(ancTraffic),
        ancVoices: ancVoices == null ? null : Math.round(ancVoices),
        ancCoverage: Math.round(ancCoverage),
...
function generateWhyAnalysis(winnerData, predecessor, rival, { maxBudget, priority, locale }) {
  ...
  if (winnerData.metrics.ancHasEvidence) {
    points.push(
      isEn
        ? `Evidence-based ANC score of ${winnerData.metrics.ancScore}/100, covering ${winnerData.metrics.ancCoverage}% of Travel/Office/Traffic/Voices environments.`
        : `Score ANC fondé sur les preuves de ${winnerData.metrics.ancScore}/100, couvrant ${winnerData.metrics.ancCoverage}% des environnements Voyage/Bureau/Trafic/Voix.`
    );
  } else if (m.anc) {
    points.push(
      isEn
