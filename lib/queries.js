...
export const getAncIntelligence = unstable_cache(async () => {
  ...
  const { data, error } = await supabase
    .from('earbuds_anc_scores')
    .select('earbud_id, anc_score, anc_travel_score, anc_office_score, anc_traffic_score, anc_voices_score, environment_count, direct_evidence_count, supporting_evidence_count, source_count');
  if (error) throw error;
  return data || [];
