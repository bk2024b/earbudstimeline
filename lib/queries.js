...
export const getPublishedGuideSlugs = unstable_cache(async () => {
  const supabase = getSupabase();
  const { data, error } = await supabase.from('guides').select('slug').eq('status', 'published');
  if (error) throw error;
  return (data || []).map((row) => row.slug);
}, ['guides-published-slugs'], { revalidate: CATALOG_REVALIDATE });
